...
for nn in range(1,2):
    ...
    # Plot 1: Total Train Loss vs. Epoch (Log Scale)
    ...
    plt.plot(data_rcnn['epoch'], data_rcnn['total_train_loss'], 'pink', label='Faster R-CNN')
    plt.yscale('log')  # Set logarithmic scale for y-axis
    plt.title(f'Total Training Loss in Task {nn}', fontsize=font_size+2)
    ...
    plt.savefig(f'{output_dir}/train_loss_nn{nn}.png')
    plt.close()

    # Plot 2: Total Validation Loss vs. Epoch (Log Scale)
    plt.figure(figsize=(8, 6))
    ...
    plt.plot(data_rcnn['epoch'], data_rcnn['total_val_loss'], 'pink', label='Faster R-CNN')
    plt.yscale('log')  # Set logarithmic scale for y-axis
    plt.title(f'Total Validation Loss in Task {nn}', fontsize=font_size+2)
    plt.xlabel('Epoch', fontsize=font_size)
